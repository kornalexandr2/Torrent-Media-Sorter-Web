...
### 2. Загрузка файлов
В папке должны находиться 5 файлов:
...
5.  `stop_words.txt` (Список стоп-слов для очистки названий)
...
## 🔍 Фильтрация и Маски (Внешние файлы)

Для тонкой настройки определения типов контента и очистки имен используются текстовые файлы. Это позволяет не загромождать основной конфиг.

### 1. `stop_words.txt` (Очистка названий)
Сюда вносятся названия релиз-групп (`lostfilm`, `alexfilm`) и другие теги, которые мешают поиску в API.
*   Слова из этого списка удаляются из названия **перед** тем, как скрипт спросит у Кинопоиска или TMDB "Что это за фильм?".
*   Каждое слово пишется с новой строки.

### 2. `masks_movies.txt` и `masks_series.txt`
Используются для определения типа контента, если API недоступно или вернуло неопределенный результат.
*   Поддерживают регулярные выражения (Regex).
*   Например, если в названии папки есть слово `trilogy`, оно будет обработано как фильм, если это прописано в `masks_movies.txt`.
...
3.  **Неверно определяет фильм/сериал или не находит данные:**
    * Попробуйте изменить порядок или наличие API в конфиге.
    * Если API не находит фильм, проверьте `stop_words.txt`. Возможно, в названии торрента есть специфический тег релиз-группы, который мешает поиску — добавьте его в этот файл.
    * Если API ошибается в типе, скрипт ориентируется на маски (`masks_*.txt`) и наличие файлов серий внутри папки.
